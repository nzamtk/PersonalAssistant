...
const useGemini = () => {
  const callGemini = async (messages, systemPrompt = null) => {
    try {
      ...
      return data.response || "申し訳ありません。応答を取得できませんでした。";
...
export default function PersonalAssistantApp() {
  ...
  const [userProfile, setUserProfile] = useState({
    name: "nzam",
    girlfriendName: "あいり", // 彼女の名前（デフォルト）
    field: "統計物理（長距離相互作用を持つ1次元イジング模型）",
    occupation: "大学院生",
    jobHunting: {
      status: "就活中",
      target: "金融IT・クオンツ・システム開発",
      companies: [],
      recentEvents: []
    },
    concerns: [],
...
キャラクター設定:
- 名前: ${userProfile.girlfriendName}（この名前で一貫して会話してください）
...
- できなかった時も責めず、次の計画を一緒に考える`;
    ...
    try {
      ...
      // キーワードベースで条件付き実行（API呼び出し削減）
      const taskKeywords = ['タスク', '面接', 'ES', 'エントリーシート', '提出', '期限', 'レポート', '締切', '締め切り', 'までに', '明日', '来週', '金曜', '火曜', '水曜', '木曜', '月曜', '土曜', '日曜'];
      const profileKeywords = ['通過', '合格', '不合格', '結果', '内定', '選考', '企業', '会社', '面接', '一次', '二次', '最終'];
      
      const hasTaskKeyword = taskKeywords.some(keyword => msgContent.includes(keyword));
      const hasProfileKeyword = profileKeywords.some(keyword => msgContent.includes(keyword));
      
      // プロフィール更新（関連キーワードがある場合のみ）
      if (hasProfileKeyword) {
        updateProfileInBackground(msgContent);
      }
      
      // タスク自動追加（関連キーワードがある場合のみ）
      if (hasTaskKeyword) {
      ...
      if (response.ok) {
        ...
        if (result.hasUpdate && result.updates) {
          ...
          setUserProfile(prev => {
            ...
            console.log('✅ プロフィール更新:', result.updates);
  ...
  const addTasksInBackground = async (userMessage) => {
    console.log('🔍 タスク抽出開始:', userMessage);
    try {
      ...
      if (response.ok) {
        ...
        if (result.hasTasks && result.tasks && result.tasks.length > 0) {
          ...
          console.log('✅ タスク自動追加:', newTasks.map(t => t.title));
          
          // 通知を表示
          alert(`🤖 タスクを${newTasks.length}件追加しました！\n\n${newTasks.map(t => '• ' + t.title).join('\n')}`);
        } else {
          console.log('ℹ️ タスクは検出されませんでした');
        }
      } else {
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className={`${cardBg} rounded-xl p-6 shadow-lg border ${borderColor}`}>
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="text-lg font-semibold mb-2">AI自動プロフィール更新</h3>
            <p className={textSecondary}>
              会話から重要な情報（就活の進捗、面接結果、企業情報など）を自動的に抽出し、
              プロフィールを更新します。更新内容はここで確認・編集できます。
            </p>
            <p className={`${textSecondary} text-sm mt-2`}>
              最終更新: {formatDate(userProfile.lastUpdated)}
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className={`block text-sm font-medium ${textSecondary} mb-1 flex items-center gap-2`}>
              💕 彼女の名前
              <span className="text-xs px-2 py-0.5 bg-pink-500 text-white rounded-full">重要</span>
            </label>
            <input
              type="text"
              value={editedProfile.girlfriendName}
              onChange={(e) => setEditedProfile({...editedProfile, girlfriendName: e.target.value})}
              placeholder="例：さくら、あかり、みお"
              className={`w-full px-4 py-2 border ${borderColor} rounded-lg bg-transparent focus:ring-2 focus:ring-pink-500 outline-none`}
            />
      ...
      <div className={`${cardBg} rounded-xl p-6 shadow-lg border ${borderColor}`}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-semibold">就活情報</h2>
          <span className="text-xs px-2 py-1 bg-pink-500 text-white rounded-full">AI自動更新</span>
        </div>
        
        {/* 応募企業 */}
        <div className="mb-6">
          <h3 className={`text-sm font-semibold ${textSecondary} mb-3`}>応募企業</h3>
          {userProfile.jobHunting.companies.length === 0 ? (
            <p className={`${textSecondary} text-sm italic`}>
              まだ企業情報が記録されていません。就活の進捗を話すと自動的に記録されます。
            </p>
          ) : (
            <div className="space-y-3">
        ...
        {userProfile.jobHunting.recentEvents.length > 0 && (
          <div className="mb-6">
            <h3 className={`text-sm font-semibold ${textSecondary} mb-3`}>最近のイベント</h3>
      ...
      {userProfile.concerns.length > 0 && (
        <div className={`${cardBg} rounded-xl p-6 shadow-lg border ${borderColor}`}>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-semibold">現在の悩み・課題</h2>
            <span className="text-xs px-2 py-1 bg-pink-500 text-white rounded-full">AI自動更新</span>
          </div>
          <ul className="space-y-2">
      ...
      {userProfile.strengths.length > 0 && (
        <div className={`${cardBg} rounded-xl p-6 shadow-lg border ${borderColor}`}>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-semibold">あなたの強み</h2>
            <span className="text-xs px-2 py-1 bg-pink-500 text-white rounded-full">AI自動更新</span>
          </div>
          <ul className="space-y-2">
